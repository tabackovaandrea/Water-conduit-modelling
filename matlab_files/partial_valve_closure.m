...
C= 0.1425; % Parametr ventilu

% Parametry částečného zavírání ventilu
t_start = 20; % sekund

Q_start = 4; % m3/s
Q_cil = 3; % m3/s průtok, na kterém končí přivírání
...
% Podmínka komín K3
 ...
 % Podmínka ventil napravo -  ČÁSTEČNÉ ZAVÍRÁNÍ 
         if time<t_start
         ...
         elseif time<t_start+t_trvani
           ...
           koef_privirani = (time - t_start) / t_trvani;
           Q(j+1, N_pol+N8+N9+N10+N11+N12+N13) = Q_start - (Q_start - Q_cil) * koef_privirani;
...
%vidObj = VideoWriter('Partial_valve_closure','MPEG-4');
...
set(f_subplot_Q_konec,'position',[0.033 0.54 0.95 0.4]);
...
set(f_subplot_H_konec,'position',[0.033 0.06 0.95 0.4]);
