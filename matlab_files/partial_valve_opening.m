...
C= 0.06277; % Parametr ventilu

% Parametry částečného otevírání ventilu
t_start = 20; % sekund

Q_start = 2; % m3/s
Q_cil = 3; % m3/s průtok, na kterém končí otevírání
...
a6 = D_K6/(sin(alpha6)*2); % Klavní poloosa
...
t_trvani = dt*round(60/dt); % sekund
...
Hk3(1,1)=H(1,N1+N2);
Hk5=zeros(n,1);
Hk5(1,1)=H(1,N1+N2+N3+N4+N5);
Hk6=zeros(n,1);
Hk6(1,1)=H(1,N1+N2+N3+N4+N5+N6);
Hk8=zeros(n,1);
Hk8(1,1)=H(1,N_pol);
Hk11=zeros(n,1);
Hk11(1,1)=H(1,N_pol+N8+N9+N10);
Hk12=zeros(n,1);
Hk12(1,1)=H(1,N_pol+N8+N9+N10+N11);
Hk13=zeros(n,1);
Hk13(1,1)=H(1,N_pol+N8+N9+N10+N11+N12);
...
% Podmínka komín K3
 ...
 % Podmínka ventil napravo - ČÁSTEČNÉ OTEVÍRÁNÍ 
         if time<t_start
         ...
         elseif time<t_start+t_trvani

           koef_otevirani = (time - t_start) / t_trvani;
           Q(j+1, N_pol+N8+N9+N10+N11+N12+N13) = Q_start + (Q_cil - Q_start) * koef_otevirani;

           H(j+1,N_pol+N8+N9+N10+N11+N12+N13)=B/A*(-Q(j,N_pol+N8+N9+N10+N11+N12+N13)+Q(j,N_pol+N8+N9+N10+N11+N12+N13-1) + A/B * H(j,N_pol+N8+N9+N10+N11+N12+N13-1) ...
               - dt*f/(2*A*D)*Q(j,N_pol+N8+N9+N10+N11+N12+N13-1)*abs(Q(j,N_pol+N8+N9+N10+N11+N12+N13-1)));
         
         else
           Q(j+1, N_pol+N8+N9+N10+N11+N12+N13)=1/(1+dt/Tf)*(Q(j, N_pol+N8+N9+N10+N11+N12+N13)+dt/Tf*Q_cil);

           H(j+1,N_pol+N8+N9+N10+N11+N12+N13)=-B/A*Q(j+1, N_pol+N8+N9+N10+N11+N12+N13)+B/A*(Q(j,N_pol+N8+N9+N10+N11+N12+N13-1) + A/B * H(j,N_pol+N8+N9+N10+N11+N12+N13-1) ...
               - dt*f/(2*A*D)*Q(j,N_pol+N8+N9+N10+N11+N12+N13-1)*abs(Q(j,N_pol+N8+N9+N10+N11+N12+N13-1)));
          end
...
%vidObj = VideoWriter('Partial_valve_opening','MPEG-4');
...
set(f_subplot_Q_konec,'position',[0.035 0.54 0.95 0.4]);
...
set(f_subplot_H_konec,'position',[0.035 0.06 0.95 0.4]);
